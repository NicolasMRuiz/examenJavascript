...
función que permita obtener un nuevo arreglo de objetos que conste con el nombre del usuario,
su email, su dirección completa, su compañía, los títulos de sus películas vistas y la calificación
en promedio de todas sus películas vistas. Formato del objeto y nombre de la función:
{
name: “Juan Pérez”,
email: “juanperez23@gmail.com”,
address: “Calle, departamento, ciudad”,
company: “Mario Bros.”,
movies: [ “Titanic”, “Blade”],
rate: 7.4
}
...
function listUsersWithMovies(users, movies) {
    ...
    for (let user of users) {
        let userMovies = movies.filter(movie => movie.userId === user.id);

        let totalRate = userMovies.reduce((sum, movie) => sum + movie.rate, 0);
        let averageRate = totalRate / userMovies.length;
        ...
        let userData = {
            ...
            movies: userMovies.map(movie => movie.movie),
            rate: averageRate.toFixed(1) // Redondear a un decimal
        };
...
let userListWithMovies = listUsersWithMovies(users, movies);
console.log(userListWithMovies);